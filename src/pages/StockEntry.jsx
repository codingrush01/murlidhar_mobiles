...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (inv) {
      ...
      setPrice(inv.price);
      setNewQty(""); // user must enter only new qty
    } else {
  ...
  const handleSaveStock = async () => {
    ...
    if (!selectedShop || !selectedBrand || !modelName || !selectedCat || !price) {
      return toast.error("Complete all fields");
    }
    ...
    if (existingInventory) {
      ...
      // Show toast for merge
      toast.info(`Merged ${oldQty} (existing) + ${addedQty} (new) = ${finalQty}`);
    } else {
      finalQty = addedQty || Number(qty);
    }
  ...
  >
    {s.shopName}
      ...
      <div className="space-y-1">
        <Label>
          {existingInventory ? "Add Quantity" : "Quantity"}
        </Label>
        <Input
          type="number"
          value={newQty}
          onChange={(e) => setNewQty(e.target.value)}
          placeholder={existingInventory ? "Enter additional quantity" : "Quantity"}
        />
      </div>
